 ...
 */
void MostrarMenu() {
  std::cout << "Este programa se centra en la implementación de las tablas hash.\n";
  std::cout << "Opciones:";
...
int main(int argc, char* argv[]) {
  ...
  ModuloFunction<Persona> dispersion(tableSize);

  // Crear la función de exploración
  LinearExploration<Persona> exploration;

  // Crear la tabla hash según el tipo de dispersión seleccionado
  if (hashType == "open") {
    HashTable<Persona, dynamicSequence<Persona>> hashTable(tableSize, dispersion,
                                                   exploration);
    std::cout << "Tabla hash configurada con dispersión abierta.\n";

    // Crear múltiples NIFs e insertarlos
    std::vector<Persona> nifVector;
    Persona Juan(11111111, "Juan", 20), Ana(22222222, "Ana", 25);
    nifVector.push_back(Juan);
    nifVector.push_back(Ana);
    hashTable.insert(Juan);
    ...
    for (size_t i = 0; i < nifVector.size() / 2; ++i) {
      Persona randomNif = nifVector[rand() % nifVector.size()];
      std::cout << "Persona: " << randomNif << " "
    ...
    HashTable<Persona, staticSequence<Persona, 5>> hashTable(tableSize, dispersion,
                                                     exploration);
