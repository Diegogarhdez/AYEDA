...
BigUnsigned<Base>::BigUnsigned(const unsigned char* puntero) : numerin_() {
  ...
  for (size_t i = length; i > 0; --i) {
    ...
    if (std::isdigit(c)) {
      ...
      throw std::invalid_argument("Entrada no válida: caracteres fuera del rango permitido.");
    }

    if (valor >= Base) {
      throw std::invalid_argument("Entrada no válida: dígito fuera del rango de la base.");
    }
